 ...
 * provided to you
 * ("License"). Unless the License provides otherwise, you may not use, modify,
...
CommandResponse CndpPort::Init(const bess::pb::CndpPortArg &arg) {
  ...
  if (!cndp_instance_->IsConfigured()) {
    const char *err_msg = "CNDP configuration failed";
    LOG(ERROR) << err_msg;
    return CommandFailure(EINVAL, "%s", err_msg);
  }
  ...
  if (lport_socket_id_ < 0) {
    ...
    LOG(INFO) << "CPU Socket id for lport = " << lport_socket_id_;
  }
...
int CndpPort::RecvPackets(queue_t qid __cne_unused, bess::Packet **pkts,
  ...
  // Keep below commented for debugging.
  /*if (num_pkts_read > 0) {
    LOG_FIRST_N(INFO, 1) << "Num packets read = " << num_pkts_read;
    LOG_FIRST_N(INFO, 1) << "BESS pkt data len = " << pkts[0]->data_len();
    LOG_FIRST_N(INFO, 1) << "BESS pkt total len = " << pkts[0]->total_len();
    LOG_FIRST_N(INFO, 1) << "BESS pkt Dump(); = " << pkts[0]->Dump();
  }*/
...
uint16_t CndpPort::XskdevSendPackets(struct fwd_port *fport,
  ...
  if (same_pool) {
    ...
    // Keep below commented for debugging.
    /*if (n_pkts > 0) {
      LOG_FIRST_N(INFO, 1) << "Num packets to send  = " << n_pkts;
      LOG_FIRST_N(INFO, 1)
          << "BESS alloc pkt data len = " << alloc_pkts[0]->data_len();
      LOG_FIRST_N(INFO, 1)
          << "BESS alloc pkt total len = " << alloc_pkts[0]->total_len();
      LOG_FIRST_N(INFO, 1)
          << "BESS alloc pkt Dump(); = " << alloc_pkts[0]->Dump();
    }*/
...
void CndpPort::CndpStats(bool reset) {
  ...
  // Log first 2 packets. Keep below for debugging.
...
bool CndpSingleton::CreatePktmbufPool(jcfg_umem_t *umem, jcfg_info_t *j) {
  ...
  if (umem->mm == NULL)
    CNE_ERR_RET("**** Failed to allocate mmap memory %ld\n",
                (uint64_t)umem->bufcnt * (uint64_t)umem->bufsz);
...
int CndpSingleton::CreateXskdevSocket(jcfg_umem_t *umem, jcfg_lport_t *lport,
  ...
  if (bess_pool == nullptr) {
    LOG(ERROR) << "Couldn't get CNDP MemPool";
    return -1;
  }
  ...
  LOG(INFO) << "RTE memzone Umem addr = " << mp_umem_addr_base_align;
  LOG(INFO) << "RTE memzone Umem len = " << mp_umem_len;
  LOG(INFO) << "Populated Size = " << mp->populated_size;
  LOG(INFO) << "Align = " << align;
  LOG(INFO) << "RTE memzone hugepaze sz = " << mp->mz->hugepage_sz;
  LOG(INFO) << "RTE memzone socket id = " << mp->mz->socket_id;
  LOG(INFO) << "Element size = " << mp->elt_size;
  LOG(INFO) << "header size = " << mp->header_size;
  LOG(INFO) << "trailer size = " << mp->trailer_size;
  LOG(INFO) << "Frame size = " << umem_framesize;
  LOG(INFO) << "buf_headroom = " << buf_headroom;
  LOG(INFO) << "Packet size = " << sizeof(bess::Packet);
  LOG(INFO) << "rte_mbuf size = " << sizeof(rte_mbuf);
  LOG(INFO) << "rte_mbuf private size = " << rte_pktmbuf_priv_size(mp);
  LOG(INFO) << "Num memory chunks = " << mp->nb_mem_chunks;
  LOG(INFO) << "Mempool max Size = " << mp->size;
  LOG(INFO) << "BESS packet pool size " << bess_pool->Size();
  LOG(INFO) << "BESS packet pool capacity " << bess_pool->Capacity();
